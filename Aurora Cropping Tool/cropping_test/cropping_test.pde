...
int evaluate_cbox_quality()
{
    ...
    avg_green /= n;
    avg_blue  /= n;
    if ((avg_red+avg_green+avg_blue)/3 < 60 || avg_green < avg_red/2 || has_black_pixel)
    {
...
{
    cbox_x += ceil(cbox_w*0.1*steps*1);
    if (cbox_x + cbox_w >= current_image.width)
    {
        cbox_y += ceil(cbox_h*0.1*steps*1);
        cbox_x  = 0;
    }
    if (cbox_y + cbox_h >= current_image.height)
    {
        ...
        if (cbox_w < current_image.width/300)
        {
...
void process_normal_image()
{
    for (int q = 0; q < frames_per_frame; q++)
    {
        if (current_loaded == false)
        {
            ...
            int x_size = floor(current_image.width /2);
            int y_size = floor(current_image.height/2);
